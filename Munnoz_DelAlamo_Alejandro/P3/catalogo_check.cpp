...
DeclarationMatcher One_Constructor_Usuario_Matcher =
  cxxRecordDecl(
	     forEach(
		     cxxConstructorDecl(hasBodyConstructor(), unless(isImplicit())).bind("one_constructor")
		     ),
	     hasName("Usuario")
	     );
...
DeclarationMatcher One_Constructor_LineaPedido_Matcher =
  cxxRecordDecl(
	     forEach(
		     cxxConstructorDecl(hasBodyConstructor(), unless(isImplicit())).bind("constructor_lineapedido")
		     ),
	     hasName("LineaPedido")
	     );
...
DeclarationMatcher Non_Constructor_Pedido_Matcher =
  cxxRecordDecl(
	     has(
		 cxxConstructorDecl(hasBodyConstructor(), unless(isImplicit())).bind("const_pedidoarticulo")
		 ),
	     hasName("Pedido_Articulo")
	     );
	     ...
	     hasDescendant(
			   friendDecl(                                
				      unless(
						 ...
						 functionDecl(
							      hasName("operator<<"),
							      hasAncestor(cxxRecordDecl(hasName("Usuario")))
...
DeclarationMatcher DynamicCast_Pedido_Matcher =
  cxxConstructorDecl(
		  unless(isImplicit()),
		  forEachDescendant(		
				    cxxDynamicCastExpr().bind("dynamic_constructor_pedido")
						),
		  ofClass(cxxRecordDecl(hasName("Pedido")))
		  );
