...
#include <functional>	// Biblioteca necesaria para std::not1()

// Algoritmo de Luhn 
bool luhn (const Cadena& numero);

// Definición Métodos clase Numero 
...
Numero::Numero(const Cadena& digitos)
{
	...
	*i = '\0'; // Incluimos el caracter terminador en la cadena 
	...
	auto f = std::find_if(n.begin(), n.end(),std::not1(std::function<bool(char)>(EsDigito()))); // Esta funcion comprueba que todos los caracteres de la cadena son dígitos 
	if (i != f)																	// En caso de no ser dígito, lanza la excepción Incorrecto(DIGITOS) 
	...
	if(!luhn(cadenaFinal))	// Si el algoritmo de luhn devuelve falso 
...
// Definición Métodos clase Tarjeta 
...
// Definición Operadores Externos 
